...
/**
 * MuscleAtlasCanvas — DIDÁCTICO (A) · Subcutáneo y Fascia más finos
 *
 * Ajustes solicitados:
 * - El tramo "Subcutáneo" (entre SkinEnd y SubcEnd) estaba demasiado ancho.
 * - El tramo "Fascia" (entre SubcEnd y FasciaEnd) también demasiado ancho.
 *
 * Defaults nuevos (más finos, estilo paper):
 * - skinEnd:  0.06
 * - subcEnd:  0.24   (subcutáneo más fino)
 * - fasciaEnd:0.34   (fascia más fina)
 * - muscleStart:0.52 (inicio músculo antes)
 *
 * Nota: esto es guía didáctica. Si tu eco tiene tejido adiposo muy marcado,
 * puedes subir subcEnd (0.26-0.30).
 */
export default function MuscleAtlasCanvas({
  src,
  anatomyBox = { x0: 10, y0: 18, x1: 95, y1: 86 },
  layerPercents = { skinEnd: 0.06, subcEnd: 0.24, fasciaEnd: 0.34, muscleStart: 0.52 },
  className = "",
}) {
  if (!src) return null;
  ...
  const ySubcEnd = y0 + H * (layerPercents.subcEnd ?? 0.24);
  const yFasciaEnd = y0 + H * (layerPercents.fasciaEnd ?? 0.34);
  const yMuscleTop = y0 + H * (layerPercents.muscleStart ?? 0.52);
  const yBottom = y1;
  ...
  return (
    ...
    >
      ...
      {/* Labels: colocados dentro de cada banda */}
      ...
      {/* Flechas de espesores (todas más finas) */}
