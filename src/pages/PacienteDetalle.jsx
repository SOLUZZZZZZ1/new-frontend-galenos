...
function isCosmeticType(t){return String(t||'').toUpperCase().startsWith('COSMETIC');}
        ...
        {open.imagenes && (
          <div className="mt-3 space-y-3">
            ...
            {!imagingError && imaging.length === 0 ? (
              ...
              imaging.filter((img)=>!isCosmeticType(img.type)).map((img) => {
...
<section className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 space-y-4">
  <div className="flex items-center justify-between gap-4">
    ...
    <p className="text-xs text-slate-500">Antes / Después / Seguimiento (solo lectura).</p>
  </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {arr.map((img) => (
          <a key={img.id} href={img.file_path} target="_blank" rel="noreferrer"
             ...
             title={`ID ${img.id}`}>
            <img src={img.file_path} alt={`Cosmetic ${img.id}`} className="w-full h-[86px] object-cover rounded-t-lg" />
            <div className="px-2 py-1">
              <p className="text-[10px] text-slate-600">ID {img.id}</p>
              <p className="text-[10px] text-slate-500">{String(img.type || "").replace("COSMETIC_", "")}</p>
            </div>
          </a>
        ))}
      </div>
